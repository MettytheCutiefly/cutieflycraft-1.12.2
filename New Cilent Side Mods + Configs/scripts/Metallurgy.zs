...
val ore_washer = mods.gregtech.recipe.RecipeMap.getByName("orewasher");
...
for denseOre, stringOreDicts in denseMetallurgy {
	...
	for i in stringOreDicts {
		for ore in oreDict["metallurgy" + i].items{
			...
			var starOreDict = oreDict["star" + i];
			starOreDict.addAll(oreDict["ore" + i + "Squeezer"]);
...
oreDict["starThorium"].add(oreDict["oreThorium"].firstItem);

furnace.remove(<contenttweaker:tungstate_ore_cluster>*2);
#manual for titanium


# Ore washer Petrotheum

val owRecipes = ore_washer.recipes;

for owRecipe in owRecipes {
	if (owRecipe.fluidInputs[0].name == "water") {

		val input = owRecipe.inputs[0]; // list of InputIngredient
		val mInput = input.matchingItems[0];
		val realInput = mInput as IIngredient;

		var outputs as IItemStack[] = [];
		for output in owRecipe.outputs {
			outputs += output;
			// print("output1: " + output.displayName);
		}

		ore_washer.recipeBuilder()
		.inputs(mInput)
		.fluidInputs(<liquid:petrotheum> * 72)
		.outputs(outputs)
		.duration(100)
		.EUt(32)
		.buildAndRegister();
