...
val tiered_items as IItemStack[][string]= {
	...
	"wire_hull": [<gregtech:cable:71>,<gregtech:cable:18>,<gregtech:cable:26>,<gregtech:cable:1>,<gregtech:cable:74>,<gregtech:cable:195>,<gregtech:cable:307>,<gregtech:cable:308>,<gregtech:cable:354>],
	"wire_machine": [<gregtech:cable:71>,<gregtech:cable:18>,<gregtech:cable:26>,<gregtech:cable:1>,<gregtech:cable:51>,<gregtech:cable:135>,<gregtech:cable:307>,<gregtech:cable:2308>,<gregtech:cable:354>],
	...
	"transformer_coil": [null,null,<gregtech:meta_item_2:32456>,<gregtech:meta_item_2:32456>,<gregtech:meta_item_2:32483>,<gregtech:meta_item_2:32483>,<gregtech:meta_item_2:32479>,null,null],
	...
	"e_input": [<gregtech:machine:714>,<gregtech:machine:724>,<gregtech:machine:734>,<gregtech:machine:744>,<gregtech:machine:754>,<gregtech:machine:764>,<gregtech:machine:774>,<gregtech:machine:784>,<gregtech:machine:794>],
	...
	"rotor_holder": [null,null,<gregtech:machine:817>,null,null,<gregtech:machine:818>,null,null,null],
...
val tiered_results as double[string][string] = {
	...
	"transformer": {"hull":1.0D,"wire_machine":4.0D,"transformer_coil":2.0D} as double[string],
...
val yield_multipliers as double[][string] = {
	...
	"transformer_coil": [0.0D, 0.0D, 0.5D, 0.5D],
...
// some UV items need their wire_machine/wire_hull yield divided by 4
// this is pretty much the set of machines that use `wire_hull` instead of `wire_machine`
// besides cefs/ceus/battery bois/energy hatches,
// except UV hulls themselves use 4x wire
val uv_wire_mod as double[string] = {
	"arc_furnace": 0.25D,
	"plasma_furnace": 0.25D,
	"mass_fabricator": 0.25D,
	"replicator": 0.25D,
	"hull": 4.0D,
	"e_input": 4.0D,
	"e_output": 4.0D
...
for item_name, base_result_map in tiered_results {
	...
	if(item_name == "transformer"){
		print("(expect 'unregistered tiered component: transformer_coil' warnings)");
	}
	...
	for i in 0 to 9 {
		...
		if(!isNull(item_tier_i)){
			...
			for component_name, component_base_amount in base_result_map {
				...
				if(i == 7 && (component_name == "wire_hull" || component_name == "wire_machine") && (uv_wire_mod has item_name)){
					component_amount *= uv_wire_mod[item_name];
				}else if(i <= 3 && component_name == "wire_machine" && item_name == "bender"){
					component_amount *= 0.5D;//low tier bending machines use a wrench instead of the top wire
				}
			...
			if(item_name == "transformer"){
				val nextwire_guaranteed_frac as double = (item_guaranteed_multiplier*yield_multipliers["wire_machine"][2]) as double;
				val nextwire_chance as double = (item_chance_multiplier*yield_multipliers["wire_machine"][3]) as double;
				val nextwire_amount as double = 1.0D as double;
				val nextwire_item as IItemStack = tiered_items["wire_machine"][i + 1] as IItemStack;
				var nextwire_guaranteed_amount as int = (nextwire_amount*nextwire_guaranteed_frac) as int;
				if(nextwire_guaranteed_amount == 0 && nextwire_guaranteed_frac != 0 && nextwire_amount >= 1.0D){
					nextwire_guaranteed_amount += 1;
				}
				val nextwire_chanced_amount as int = (nextwire_amount - nextwire_guaranteed_amount) as int;
				
				if(nextwire_guaranteed_amount > 0){
					guaranteed_outputs += nextwire_item*nextwire_guaranteed_amount;
				}
				if(nextwire_chanced_amount > 0){
					recipeBuilder.chancedOutput(nextwire_item*nextwire_chanced_amount, (nextwire_chance*10000.0D) as int, ((1.0D-nextwire_chance)*1300.0D) as int);
				}
			}
...
val nontiered_recipes as IItemStack[][IItemStack] = {
	...
	<gregtech:machine:819>*1: [<gregtech:cable:4354>*7,<gregtech:meta_item_2:26304>*1,<draconicevolution:draconic_core>*1],
	<modulardiversity:blockdaylightdetector>*1: [<modularmachinery:blockcasing>*4,<minecraft:glowstone_dust>*8,<minecraft:comparator>*2,<minecraft:daylight_detector>*1],
	<minecraft:daylight_detector>*1: [<minecraft:glass>*3,<gregtech:meta_item_1:2201>*3,<gregtech:meta_item_1:1196>*6],
	<minecraft:observer>*1: [<gregtech:meta_item_1:2328>*6,<minecraft:redstone>*2,<gregtech:meta_item_1:2203>*1],
	<minecraft:repeater>*1: [<gregtech:meta_item_1:2328>*3,<minecraft:redstone>*3,<gregtech:meta_item_1:1196>*4],
	<minecraft:comparator>*1: [<gregtech:meta_item_1:2328>*3,<minecraft:redstone>*3,<gregtech:meta_item_1:2203>*1],
	<minecraft:cake>*2: [<harvestcraft:doughitem>*3,<minecraft:sugar>*2,<minecraft:egg>*1]
} as IItemStack[][IItemStack];

val nontiered_times as int[IItemStack] = {
	<minecraft:cake>*2: 480,
	<gregtech:machine:819>*1: 2*60*20*128
} as int[IItemStack];

for inputstack, results in nontiered_recipes {
	val recipe_time = (nontiered_times has inputstack ?
		nontiered_times[inputstack] :
		2*60*20*2
	);
	var recipeBuilder = disassembler_recipe_map.recipeBuilder()
		.duration(recipe_time)
		.EUt(128)
		.inputs(inputstack);
	var guaranteed_outputs as IItemStack[] = [] as IItemStack[];
